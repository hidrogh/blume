		...
		if (Config.userZones == true) {
			for (int i = 0; i < 50; i++) {
				if (Bukkit.getServer().getWorld("world").getPlayers().size() > 0) { //if map is not loadet, .getWorld will output a error
					...
					int rn1 = (int) (Math.random()*(MiningContainer.getMiningZonesSorted()[1].getBlockX()-MiningContainer.getMiningZonesSorted()[0].getBlockX())) + MiningContainer.getMiningZonesSorted()[0].getBlockX();
					int rn2 = (int) (Math.random()*(MiningContainer.getMiningZonesSorted()[1].getBlockY()-MiningContainer.getMiningZonesSorted()[0].getBlockY())) + MiningContainer.getMiningZonesSorted()[0].getBlockY();
					int rn3 = (int) (Math.random()*(MiningContainer.getMiningZonesSorted()[1].getBlockZ()-MiningContainer.getMiningZonesSorted()[0].getBlockZ())) + MiningContainer.getMiningZonesSorted()[0].getBlockZ();
					...
					System.out.print(Log.logInfo() + "no player on the server, mine block-check now stopped to prevent \"getWorld\" warning");
					break;
				}
			}
		}
