...
public class CommandSpawn_buyer implements CommandExecutor {

	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (sender instanceof Player) {
            Player p = (Player) sender;
            String name = Config.villager[0];
            int counter = 0;
            ...
            if (p.isOp()) {
            	for(Entity e : Bukkit.getWorld(Config.getWorldName()).getEntities()) { //kill old buyer
            		if (e.getCustomName() != null) {
            			if (e.getCustomName().equals(name)) {
                			...
                			p.sendMessage(ChatTags.logSystem() + "Old buyer villager got killed.");
                		}
            		}
            	}
                ...
                ((CraftVillager)villager).getHandle().setProfession(1); //type
            	...
            	for(Entity e : Bukkit.getWorld(Config.getWorldName()).getEntities()) { //kill old buyer
            		if (e.getCustomName() != null) {
            			if (e.getCustomName().equals(name)) {
            				...
            				ConfigSettings.setVillagerBuyer(p.getLocation());
            			}
            		}
            	}
